...
import {
  ...
  getPendingClaimsForOfficer,
  getClaimDetailForOfficer,
  submitOfficerDecision,
} from "../services/claimService";
import OfficerClaimDetail from "./OfficerClaimDetail";
...
const Dashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState("my"); // 'my' or 'officer'
  ...
  const handleTabSwitch = async (tab) => {
    ...
    if (isOfficer && !pendingLoaded) {
      try {
        setLoadingPending(true);
        const pending = await getPendingClaimsForOfficer();
  ...
  const handleDecision = async (decision) => {
    if (!selectedPendingClaim) return;
  ...
  // Logout handler passed to Navbar (auth clearing handled by authService)
  const handleLogout = () => {
    try {
      logout();
    } catch (err) {
    ...
    window.location.href = "/claimservice/login";
  };
  ...
  const renderOfficerDashboard = () => {
    const highRiskCount = pendingClaims.filter((claim) => {
      const score = claim.aiSummary?.slaRiskScore;
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
          ...
          <div className="space-y-4">
            ...
            <OfficerClaimDetail
              ...
              loading={decisionLoading || detailLoading}
              error={decisionError}
            />
          </div>
        </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-lg font-semibold ${activeTab === "my" ? "bg-indigo-600 text-white" : "bg-white text-gray-700 border border-gray-200"}`}
              >
                My Claims
              </button>
              {isOfficer && (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-semibold ${activeTab === "officer" ? "bg-indigo-600 text-white" : "bg-white text-gray-700 border border-gray-200"}`}
                >
                  Officer Dashboard
                </button>
              )}
